...
## Scripts
- `setup_xanmod.sh` installs the XanMod kernel and required build tools.
- `setup_network.sh` applies BBR/FQ, sysctl tuning, file limits, and swap setup for Marzban/Xray nodes.
- `setup_wg-easy.sh` installs Docker/curl if missing and runs the wg-easy WireGuard UI container.
